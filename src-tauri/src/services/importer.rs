...
use crate::models::config::{load_config, save_config, with_config_lock, AppConfig};
...
pub fn delete_skill(skill_id: &str, keep_symlinks: bool) -> Result<(), String> {
    with_config_lock(|| {
        ...
        let dist_ids: Vec<String> = config
            .distributions
            .iter()
...
pub fn import_local_skill(path: &str) -> Result<Vec<Skill>, String> {
    ...
    let abs_path =
        fs::canonicalize(dir).map_err(|e| format!("Failed to resolve absolute path: {e}"))?;
    ...
    with_config_lock(|| {
        ...
        for skill_md_path in skill_files {
            let skill_source_path = skill_md_path.parent().unwrap_or(&abs_path);
...
pub fn import_github_skill(url: &str) -> Result<ImportGithubResult, String> {
    ...
    let abs_path =
        fs::canonicalize(&clone_dir).map_err(|e| format!("Failed to resolve clone path: {e}"))?;
...
fn clone_repo(url: &str, dest: &Path) -> Result<(), String> {
    git2::Repository::clone(url, dest).map_err(|e| format!("Failed to clone repository: {e}"))?;
    Ok(())
}
...
fn pull_repo(repo_path: &Path) -> Result<bool, String> {
    ...
    if is_worktree_dirty(&repo)? {
        return Err("工作区存在未提交改动，请先提交或清理后再拉取更新".to_string());
    }
    ...
    let head_refname = head
        .name()
        .ok_or("HEAD has no branch reference")?
        .to_string();
    ...
    let branch = repo
        ...
        .map_err(|e| format!("Failed to find current branch: {e}"))?;

    let (remote_name, fetch_branch, fetch_refname) = match branch.upstream() {
        Ok(upstream) => {
            let upstream_refname = upstream
                .get()
                .name()
                .ok_or("Upstream branch has no reference name")?
                .to_string();
            let remote_branch = upstream_refname
                .strip_prefix("refs/remotes/")
                .ok_or("Unsupported upstream branch reference")?;
            let (remote_name, fetch_branch) = remote_branch
                .split_once('/')
                .ok_or("Unsupported upstream branch reference")?;
            (
                remote_name.to_string(),
                fetch_branch.to_string(),
                upstream_refname,
            )
        }
        Err(_) => (
    ...
    remote
        .fetch(&[fetch_branch.as_str()], None, None)
        .or_else(|_| {
            if fetch_branch == "main" {
                remote.fetch(&["master"], None, None)
            } else if fetch_branch == "master" {
                remote.fetch(&["main"], None, None)
            } else {
                Err(git2::Error::from_str("fetch failed"))
            }
        })
        .map_err(|e| format!("Failed to fetch: {e}"))?;

    let fetch_ref = repo
        .find_reference(&fetch_refname)
...
fn is_worktree_dirty(repo: &git2::Repository) -> Result<bool, String> {
...
pub fn pull_skill(skill_id: &str) -> Result<PullResult, String> {
    ...
    let skill = config
        ...
        .find(|s| s.id == skill_id)
        .ok_or("Skill not found")?;
    let github_url = skill.github_url.clone();

    let source_path = Path::new(&skill.source_path);
    // Navigate to the repo root (may be a subdirectory within the cloned repo)
    let repo_path = find_git_repo_root(source_path).ok_or("Skill is not in a git repository")?;

    // Record existing skill names before pull
    let before_skills: Vec<String> = config
        .skills
        .iter()
        .filter(|s| {
            s.source_path
                .starts_with(repo_path.to_string_lossy().as_ref())
        })
        .map(|s| s.name.clone())
        .collect();
    ...
    let after_names: Vec<String> = skill_files
        .iter()
        .filter_map(|p| parse_skill_md(p).ok())
        .map(|m| m.name.clone())
        .collect();

    let new_skills: Vec<String> = after_names
        .iter()
        .filter(|n| !before_skills.contains(n))
        .cloned()
        .collect();

    let removed_skills: Vec<String> = before_skills
        .iter()
    ...
    if !new_skills.is_empty() || !removed_skills.is_empty() {
        with_config_lock(|| {
            ...
            if !removed_ids.is_empty() {
                ...
                config
                    .distributions
                    .retain(|d| !removed_ids.contains(&d.skill_id));
...
impl<'a> DiscoveryContext<'a> {
    ...
    fn scan_global_path(&mut self, dir: &std::path::Path, tool: &Tool) {
        if self.is_ignored(dir) || !dir.exists() {
            return;
        }
    ...
    fn scan_extra_global(&mut self, eg_dir: &std::path::Path, tool: &Tool) {
        if !eg_dir.exists() || self.is_ignored(eg_dir) {
            return;
        }
        let skill_dirs = find_skill_dirs_recursive(eg_dir, 5);
        for skill_dir in skill_dirs {
            if self.is_ignored(&skill_dir) {
                continue;
            }
...
pub fn discover_skills_from_tool_paths() -> Result<Vec<Skill>, String> {
    with_config_lock(|| {
        ...
        let tool_scan_paths: Vec<(Tool, std::path::PathBuf, Vec<std::path::PathBuf>)> = config
            .tool_paths
            .iter()
            .filter_map(|(k, v)| {
...
fn find_or_import_skill(
    ...
    if let Some(existing) = config
        .skills
        .iter()
        .find(|s| s.source_path == real_path_str)
    {
...
pub fn scan_project_folder(
    ...
    let home = dirs::home_dir().unwrap_or_default();
    let global_abs_paths: Vec<std::path::PathBuf> = config
        .tool_paths
        .values()
        .flat_map(|tp| {
            let mut paths = vec![];
            ...
            if !global.is_empty() {
                paths.push(if tp.global.starts_with("~/") {
                    home.join(global)
                } else {
            ...
            for eg in &tp.extra_globals {
                ...
                if !eg_rel.is_empty() {
                    paths.push(if eg.starts_with("~/") {
                        home.join(eg_rel)
                    } else {
                        std::path::PathBuf::from(eg_rel)
                    });
                }
            }
            paths
    ...
    let tool_configs: Vec<(String, String)> = config
        .tool_paths
        .iter()
...
mod tests {
    ...
    fn test_scan_project_folder_mixed_entry_types() {
        ...
        let real_skill_dir = project_dir
            .path()
            .join(".claude")
            .join("skills")
            .join("real-skill");
        ...
        crate::test_helpers::create_skill_md(
            symlink_source.path(),
            "symlink-skill",
            "A symlinked skill",
        );
        let symlink_target = project_dir
            .path()
            .join(".claude")
            .join("skills")
            .join("symlink-skill");
        ...
        assert_eq!(
            project_dists.len(),
            2,
            "should have 2 project-scoped distributions"
        );

        // Real folder → Folder entry_type
        let real_dist = project_dists
            .iter()
            .find(|d| d.target_path.contains("real-skill"))
            .unwrap();
        assert_eq!(
            real_dist.entry_type,
            EntryType::Folder,
            "real directory should be Folder type"
        );

        // Symlink → Symlink entry_type
        let sym_dist = project_dists
            .iter()
            .find(|d| d.target_path.contains("symlink-skill"))
            .unwrap();
        assert_eq!(
            sym_dist.entry_type,
            EntryType::Symlink,
            "symlink should be Symlink type"
        );
        ...
        assert!(config
            .skills
            .iter()
            .all(|s| !s.source_path.contains("symlink-skill")
                || s.source_path == symlink_source.path().to_string_lossy().to_string()));
    }
    ...
    fn setup_discover() -> tempfile::TempDir {
        ...
        config
            .ignored_paths
            .push("~/.claude/plugins/marketplaces".to_string());
        save_config(&config).unwrap();
        tmp
    }
    ...
    fn test_discover_skills_from_tool_paths_real_directory() {
        ...
        let tool_skills_dir =
            std::path::PathBuf::from(format!("{tmp_path}/claudecode/global/skills/"));
        ...
        let dist = config
            .distributions
            .iter()
            .find(|d| d.skill_id == imported[0].id)
            .unwrap();
        assert_eq!(
            dist.entry_type,
            EntryType::Folder,
            "real directory should create Folder distribution"
        );
    ...
    fn test_discover_skills_from_tool_paths_symlink() {
        ...
        assert_eq!(
            dist.entry_type,
            EntryType::Symlink,
            "symlink in tool path should create Symlink distribution"
        );

        // Skill source_path should be the real (canonicalized) path
        let expected_real = fs::canonicalize(real_skill.path()).unwrap();
        assert_eq!(
            imported[0].source_path,
            expected_real.to_string_lossy().to_string(),
    ...
    fn test_discover_skills_from_tool_paths_idempotent() {
        ...
        assert!(
            second.is_empty(),
            "second discovery should find no new skills"
        );
    ...
    fn test_delete_all_skills_clears_records() {
        ...
        assert!(
            config.distributions.is_empty(),
            "all distributions should be removed"
        );
    }
    ...
    fn test_delete_all_skills_removes_symlinks() {
        ...
        assert!(
            ...
            "symlink should be removed from filesystem"
        );
        let config = crate::models::config::load_config().unwrap();
        assert!(config.skills.is_empty());
        assert!(config.distributions.is_empty());
    ...
    fn test_delete_all_skills_preserves_symlinks_when_flag_set() {
        ...
        assert!(
            symlink_path.symlink_metadata().is_ok(),
            "symlink should be preserved with keep_symlinks=true"
        );
        let config = crate::models::config::load_config().unwrap();
        assert!(
            config.skills.is_empty(),
            "skill records should still be cleared"
        );
        assert!(
            config.distributions.is_empty(),
            "distribution records should still be cleared"
        );
    }
    ...
    fn test_delete_all_skills_clears_project_skill_ids() {
        ...
        assert!(
            ...
            "project.skill_ids should be cleared"
        );
    }
    ...
    fn test_delete_all_skills_preserves_folder_directories() {
        ...
        assert!(
            real_dir.path().exists(),
            "real directory should not be deleted"
        );
        assert!(
            real_dir.path().join("data.txt").exists(),
    ...
    fn make_fast_forward_fixture(branch: &str) -> GitFixture {
        let tmp = tempfile::tempdir().unwrap();
        ...
        let source = git2::Repository::init_opts(&source_dir, &opts).unwrap();
        fs::write(source_dir.join("file.txt"), "v1").unwrap();
        commit_all(&source, "initial");
        let remote_url = bare_dir.to_string_lossy().to_string();
        let mut remote = source.remote("origin", &remote_url).unwrap();
        remote
            .push(&[format!("refs/heads/{branch}:refs/heads/{branch}")], None)
            .unwrap();
        bare.set_head(&format!("refs/heads/{branch}")).unwrap();

        git2::Repository::clone(&remote_url, &clone_dir).unwrap();
    ...
    fn push_source_update(fixture: &GitFixture, content: &str) {
        let source = git2::Repository::open(&fixture.source_dir).unwrap();
        fs::write(fixture.source_dir.join("file.txt"), content).unwrap();
        commit_all(&source, "update");
        let mut remote = source.find_remote("origin").unwrap();
        remote
            .push(
                &[format!(
    ...
    #[test]
    fn test_pull_repo_fast_forward_main_branch() {
        let fixture = make_fast_forward_fixture("main");
        push_source_update(&fixture, "v2");

        let updated = pull_repo(&fixture.clone_dir).unwrap();

        assert!(updated);
        assert_eq!(
            fs::read_to_string(fixture.clone_dir.join("file.txt")).unwrap(),
            "v2"
        );
    }

    #[test]
    fn test_pull_repo_fast_forward_master_branch() {
        let fixture = make_fast_forward_fixture("master");
        push_source_update(&fixture, "v2");

        let updated = pull_repo(&fixture.clone_dir).unwrap();
        let clone = git2::Repository::open(&fixture.clone_dir).unwrap();
    ...
    #[test]
    fn test_pull_repo_rejects_dirty_worktree() {
        let fixture = make_fast_forward_fixture("main");
        push_source_update(&fixture, "remote-v2");
        fs::write(fixture.clone_dir.join("file.txt"), "local dirty").unwrap();

        let result = pull_repo(&fixture.clone_dir);

        assert!(result.is_err());
        assert!(result.unwrap_err().contains("未提交改动"));
        assert_eq!(
            fs::read_to_string(fixture.clone_dir.join("file.txt")).unwrap(),
            "local dirty"
        );
    ...
    fn test_pull_skill_removed_skills_soft_deleted() {
        ...
        let config = load_config().unwrap();
        assert!(
            config.skills.iter().any(|s| s.id == id_a),
            "skill-a should remain"
        );
        assert!(
            !config.skills.iter().any(|s| s.id == id_b),
            "skill-b should be removed"
        );
        assert!(
            config.distributions.is_empty(),
            "distribution for skill-b should be removed"
        );
        let project = config.projects.iter().find(|p| p.id == project_id).unwrap();
        assert!(
            !project.skill_ids.contains(&id_b),
            "skill-b should be removed from project"
        );
    }
}
